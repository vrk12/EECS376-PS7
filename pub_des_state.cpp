...
void DesStatePublisher::initializePublishers() {
    ...
    //my new subscriber
    lidar_alarm_subscriber_ = nh_.subscribe("lidar_alarm",1, &DesStatePublisher::lidarAlarmCallback, this);
}
...
// --------------put in the CALLBACK for the alarm here------------
void DesStatePublisher::lidarAlarmCallback(const std_msgs::Bool& lidar_alarm_data){
    lidar_alarm_triggered = lidar_alarm_data.data;

    if (lidar_alarm_triggered)
        ROS_WARN("LIDAR ALARM HAS BEEN TRIGGERED");
}
    ...
    switch (motion_mode_) {
        ...
        case HALTING: //e-stop service callback sets this mode
            ...
            if (traj_pt_i_ >= npts_traj_) { //here if completed all pts of braking traj
                ...
                // I think add stuff here to account for the e stop                    
            }
            break;
