...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    // create a valid braking trajectory whether the robot is spinning or moving forward or both

    nav_msgs::Odometry currentState;

    int vec_of_states_size = vec_of_states.size();
    
    ROS_INFO("DOING STUFF");

    for (int i = 0 ; i < vec_of_states_size ; i++){
    	int xDiff = start_pose.pose.position.x - vec_of_states[i].pose.pose.position.x;
    	int yDiff = start_pose.pose.position.y - vec_of_states[i].pose.pose.position.y;
    	int zDiff = start_pose.pose.position.z - vec_of_states[i].pose.pose.position.z; // might be orientation?

    	if (abs(xDiff) < 0.1 && abs(yDiff) < 0.1 && abs(zDiff) < 0.1){
    		currentState.pose.pose = vec_of_states[i].pose.pose;
    		currentState.twist.twist = vec_of_states[i].twist.twist;
    		break;
    	}
    }
    ...
    bool isSpinning = abs(currentState.twist.twist.angular.z) >= 0.1;
    bool isMovingForward = abs(currentState.twist.twist.linear.x) >= 0.1;

    nav_msgs::Odometry my_des_state;

    double my_psi_des = convertPlanarQuat2Psi(currentState.pose.pose.orientation);
    double my_speed_des = currentState.twist.twist.linear.x;


    if (isSpinning){
    	double my_t_ramp = abs(my_des_state.twist.twist.angular.z) / alpha_max_;
    	double my_omega_des = my_des_state.twist.twist.angular.z;
    	double accelerationAmount = my_omega_des > 0 ? alpha_max_ : -1 * alpha_max_;


    	for (int i = 0 ; i < ceil(my_t_ramp / dt_) ; i++){
    		my_omega_des -= accelerationAmount*dt_;
    		my_des_state.twist.twist.angular.z = my_omega_des;
    		my_psi_des += my_omega_des*dt_;
    		my_des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(my_psi_des);
    		vec_of_states.push_back(my_des_state);
    	}
    	...
    	for (int i = 0 ; i < ceil(my_t_ramp / dt_) ; i++){
    		my_speed_des -= accel_max_ * dt_;
    		my_des_state.twist.twist.linear.x = my_speed_des;

    		my_des_state.pose.pose.position.x += my_speed_des * dt_ * cos(my_psi_des);
    		my_des_state.pose.pose.position.y += my_speed_des * dt_ * sin(my_psi_des);
